...
compareReferenceExonsInData <- function(data) {
    junctionQuant <- data[[1]]$`Junction quantification`
    annot <- loadAnnotation(listSplicingAnnotations()[[3]])
...
res$TCGA$cmp  <- compareReferenceExonsInData(res$TCGA$data)
print(all(res$TCGA$cmp))

res$SRA$data <- loadSRAproject("SRP053101")
res$SRA$cmp  <- compareReferenceExonsInData(res$SRA$data)
print(all(res$SRA$cmp))

options(timeout=1000)
res$GTEx$data <- loadGtexData(tissue="Adipose Tissue")
res$GTEx$cmp  <- compareReferenceExonsInData(res$GTEx$data)
