...
## Faster subsetting
```{r subsetting}
subsetting <- function(size) {
    df <- data.frame(age=sample(1:65, 1e2, replace=TRUE),
                     x=rnorm(size), y=rpois(size, 25))
    ...
    res <- microbenchmark(times=10,
                          which=df[which(df$age>5),],
                          subset=subset(df, age>5), 
                          dataframe=df[df$age>5,],
                          datatable=dt[age > 5])
    autoplot(res)
}
# testing with a smaller dataframe
subsetting(size = 1e2)
# testing with a larger dataframe
subsetting(size = 1e4)
```

It seems datatables only bring an improvement when subsetting larger dataframes.
...
```dt[ , 3, with=FALSE]```. That's rather laborious, so we need to be sure we
want to support datatables in the code or not.

### Subsetting data.frame by column and row
```{r subset by column and row}
random <- data.frame(replicate(10,sample(0:100,1000,rep=TRUE)))
res <- microbenchmark(colName1 = random[["X7"]][400:500],
                      colName2 = random[, "X7"][400:500],
                      colName3 = random[400:500, ]["X7"],
                      colName4 = random[400:500, "X7"],
                      colNo1 = random[[7]][400:500],
                      colNo2 = random[, 7][400:500],
                      colNo3 = random[400:500, ][7],
                      colNo4 = random[400:500, 7])
ggplot2::autoplot(res)

res <- microbenchmark(colName2 = random[, c("X3", "X7")][400:500, ],
                      colName3 = random[400:500, ][, c("X3", "X7")],
                      colName4 = random[400:500, c("X3", "X7")],
                      colNo2 = random[, c(3, 7)][400:500, ],
                      colNo3 = random[400:500, ][, c(3, 7)],
                      colNo4 = random[400:500, c(3, 7)])
ggplot2::autoplot(res)

# Transposing the data.frame... too slow :(
random <- as.data.frame(t(random))
res <- microbenchmark(colName2 = random[c("X3", "X7"), ][400:500],
                      colName3 = random[, 400:500][c("X3", "X7"), ],
                      colName4 = random[c("X3", "X7"), 400:500],
                      colNo2 = random[c(3, 7), ][400:500],
                      colNo3 = random[, 400:500][c(3, 7), ],
                      colNo4 = random[c(3, 7), 400:500])
autoplot(res)
```

## Creating a sequence of numbers from 1 to y
```{r number sequence}
up <- sample(10**3)
res <- microbenchmark(1:length(up),
                      seq_along(up))
autoplot(res)
```
...
```{r remove NAs}
df <- read.table(text = "1  2  3  NA 5
                         a  b  c  d  e
                         NA NA NA NA NA
                         5  4  NA 2  1")
res <- microbenchmark(
    apply = apply(df, 1, function(x) all(is.na(x))),
    rowSums = rowSums(!is.na(df)) == 0)
autoplot(res)
```

## Sorting unique values
```{r sort unique}
a <- rep(c("A", "B", "C", "D", NA), 1000)
a <- sample (a) # randomise order

res <- microbenchmark(sort(unique(a)),
                      # We can set number of unique items expected, but this
                      # doesn't seem to improve much for such a low number
                      sort(unique(a, nmax=5)),
                      unique(sort(a)),
                      unique(sort(a), nmax=5))
autoplot(res)
```