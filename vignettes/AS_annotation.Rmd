...
```{r, eval=FALSE}
types <- c("AFE", "SE", "MXE", "A5SS", "A3SS", "RI", "TandemUTR")
...
suppa.path <- "~/Documents/psi_calculation/suppa/suppaEvents/hg19_%s.ioe"
...
mats.path <- "~/Documents/psi_calculation/mats_out/ASEvents/fromGTF.%s.txt"
...
A simple approach would be for each group of columns, look at the other group.
First of all, let's try with skipping events.
```{r}
...
# More interestingly, let's see how fast can we subset each event type
microbenchmark(mats <- split(matsEvents, matsEvents$Event.type),
               mats <- dlply(matsEvents, .(Event.type)))

# All right... let's separate all events 
events <- list(matsEvents, misoEvents, suppaEvents, vastToolsEvents)
events <- rbind.fill(events)
events <- dlply(events, .(Program, Event.type))
names(events)

# mats$A1.start <- as.integer(mats$A1.start) + 1

a <- dplyr::full_join(suppa, mats, by=c("Strand",
                                        "C1.end", "A1.end", "A1.start"))
# merge(suppa, mats, by = c("C1.end", "A1.end", "A1.start"), all = TRUE)
```
...
junctions.files <- lapply(junctions, read.delim2)
```
