...
res <- microbenchmark(lapply = lapply(l, fivenum),
...
res <- microbenchmark(
  ...
  all(as.character(event) == as.character(validator)),
  identical(as.character(event), as.character(validator)),
  all.equal(as.character(event), as.character(validator))
)
autoplot(res)
```