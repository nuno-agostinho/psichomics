    ...
    fluidRow(
        shiny::column(3, radioButtons(
            ...
            choices = list("Choice 1" = 1,
                           "Choice 2" = 2,
                           "Choice 3" = 3), selected = 1)),
        shiny::column(3, selectInput(
            "select", label = h3("Select box"),
            choices = list("Choice 1" = 1,
                           "Choice 2" = 2,
                           "Choice 3" = 3), selected = 1)),
        shiny::column(3,
...
server <- function(input, output, session) {
    obsB <- observe({
        # dismiss same alert or it won't update
        closeAlert(session, "exampleAlert")
        if (input$slider1 > 90)
            createAlert(session, "alert", "exampleAlert", title = "Wow",
                        content = "Over 90!", style = "success")
