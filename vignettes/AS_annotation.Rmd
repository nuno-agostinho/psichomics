...
# List of lists: 308 seconds // 191 Mb (commit 3e702152)
```
...
One thing important to note is that ```misoEvents``` is using 11.2GB of memory.
Why is that? We're holding all unparseable events as character strings and AFE
and ALE events have many unrecognisable events. If we don't store those (by
commenting the command ```parsed[["MISO event"]] <- event```), ```misoEvents``` 
has now lowered to 3GB of memory. And what if we don't save any unrecognised
events? It's now using 23.9 MB of memory. Much better!

Also, this is slow. Mainly because getting two indices from a data frame is slow
in R. One way to improve time performance is to use the package ```data.table```
instead.

> TODO: try with data.table
...
suppa.hg19 <- lapply(c("A3", "A5", "AF", "AL", "MX", "RI", "SE"), 
                     function(type) read_path(suppa.path, type))
suppa.hg19 <- rbind.fill(suppa.hg19)
eventsID <- as.character(suppa.hg19$event_id)
suppaEvents <- parseSuppaEventID(eventsID, progress = TRUE)
# List of lists: 20.5 seconds // 342 Mb (commit 3e702152)
...
# List of lists: 63.3 seconds // 243.2 Mb (commit 3e702152)
# Data frame:    0.42 seconds //  17.9 Mb
...
types <- c("ALT3", "ALT5", "COMBI", "IR", "MERGE3m", "MIC", rep(c("EXSK", "MULTI"), 2))
num <- c(rep("", 6), rep(".1", 2), rep(".2", 2))
vastTools.hg19 <- lapply(1:length(types), 
                         function(i) read_path(vastTools.path, types[i], num[i],
                                               row.names=NULL))

vastToolsEvents <- lapply(vastTools.hg19, parseMultipleVastToolsEvents,
                          progress = TRUE)
# List of lists: 377.7 seconds // 1 Gb (commit 3e702152)

tmp <- list()
for (i in 1:length(vastToolsEvents))
    tmp <- c(tmp, vastToolsEvents[[i]])
vastToolsEvents <- tmp
...
length(vastToolsEvents)               # all event types:    504494
number(vastToolsEvents, "SE")         # skipped exons:      301155
number(vastToolsEvents, "AFE")        # alt. first exons:        0
number(vastToolsEvents, "ALE")        # alt. last exons:         0
number(vastToolsEvents, "MXE")        # mutually exclusive:      0  
number(vastToolsEvents, "A5SS")       # alt. 5 splice site:  15835
number(vastToolsEvents, "A3SS")       # alt. 3 splice site:  20860
number(vastToolsEvents, "RI")         # intron retention:   166644
...
Before anything, all time and memory was registred.

So, let's first try to get MATS events to a data frame instead of a list of 
lists since MATS is the easiest to parse. There's a clearer improvement!
# Times of parseMatsEvent(event) and memory size of result
# List of lists: 63.3 seconds // 243.2 Mb (commit 3e702152)
# Data frame:    0.42 seconds //  17.9 Mb (commit f903e2f3)
