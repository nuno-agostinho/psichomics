...
getEventData <- function(event=NULL, data=NULL) {
    ...
    eventData <- returnEventData(attr(data, "rowData"))
    if (is.null(eventData)) {
        eventData <- returnEventData(attr(data, "eventData"))
    }
    if (is.null(eventData)) {
        eventData <- returnEventData(data)
    }
    if (is.null(eventData)) {
        eventData <- returnEventData(attr(event, "eventData"))
    }
    return(eventData)
}
