...
the loaded datasets. Notes the following: 
...
impact in the subsequent analysis available in the application.

```{r, echo=FALSE, fig.retina=NULL, out.width='270pt'}
...
2. To create groups of patients based on tumour stage:
    ...
    iv).
3. To create groups of patienties based on gender:
    1. Click on the field below **Select column**.
...
```{r, echo=FALSE, fig.retina=NULL, out.width='480pt'}
...
[^1]: Missing values are replaced with the median value from the respective 
event.
...
normal tissue, if available) or stage of the disease. For now, select **Sample
types**.
