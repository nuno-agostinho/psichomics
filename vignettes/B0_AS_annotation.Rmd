...
annotation <- getParsedAnnotation()
annot2 <- NULL
...
# Let's compare!
join <- joinAnnotation(annotation)
join2 <- joinAnnotation(annot2)
...
annot <- lapply(names(join2), 
                function(i) { 
                    join2[[i]][, c(grep("Event.ID", names(join[[i]]), value = TRUE), 
                                   "Chromosome", "Strand",
                                   getCoordinates(i))]
                })
...
for (type in names(join)) {
    print(type)
    vennEvents(join, type)
    title(paste(type))
    vennEvents(join2, type)
