---
title: 'Command-line interface (CLI) tutorial'
...
* `plotSplicingEvent`: Plot the representation of given alternative splicing events
...
* `getTCGAcohorts`: Query the Firebrowse web API for TCGA cohorts
* `getTCGAdataTypes`: Query the Firebrowse web API for TCGA data types
* `getTCGAdates`: Query the Firebrowse web API for processing dates
* `loadTCGAdata`: Download and load TCGA data through the Firebrowse web 
API
* `parseTCGAsampleInfo`: Parse sample information from TCGA samples

**GTEx**

* `getGtexDataTypes`: Check available data types from GTEx
...
groups (one reference group and another containing remaining elements) and 
provided groups
...
data from [TCGA][TCGA], as required to run the downstream analyses. Before
...
transcript annotation. Please read
[Preparing alternative splicing annotations][annotation-tutorial].
...
quantification) can be automatically retrieved and loaded by following these
commands:
...
other SRA projects and user-provided data can be manually aligned using a
splice-aware aligner and loaded by following the instructions in
...
For instance, to load [GTEx][GTEx] files via local files, create a directory
called **GTEx**, put all GTEx files within that folder and type these commands:
