#' Get rows of a data frame between two row indeces
#'
...
#' in the data frame
#' 
#' @return Data frame between two row indeces (returns NA if the first row index
#' is NA)
getRowsData <- function(i, data, first_row, last_row) {
  first <- first_row[i]
  last  <- last_row[i]
  if (is.na(first)) {
...
#'
#' @return Data frame of the matching event (or NA when nothing is matched)
#' @export
#'
#' @examples
...
#' IDcolumn <- 9
#' matchMisoEventID(eventID, annotation, IDcolumn)
matchMisoEventID <- function(eventID, annotation, IDcolumn) {
  ...
  ret <- lapply(1:(length(index)), getRowsData, annotation, index,
                next_index - 1)
  return(ret)
}
...
list_mRNA <- function(event) {
  ...
  mRNA <- lapply(1:(length(mRNA_index)), getRowsData,
