...
![Splicing event types](https://bitbucket-assetroot.s3.amazonaws.com/repository/4gEBMd/2185438774-Slide1.jpg?Signature=a%2Bb%2FZ7ElgJAgUqweGpPNSP%2FPP0Y%3D&Expires=1452974150&AWSAccessKeyId=AKIAIWY5XSVPZPDQYRQQ)
...
what I thought before. Now those cases have separate coordinates for the
...
> data frame to store the information pertaining to each event. I'll try to create
> a list at first but maybe it would be better to have a class for events. This
> way, the attributes would be much clearer to others. But really, would it be
> that different if we take into account that the documentation explains the data
> structure anyway? And is there any impact in the performance of the same script
> using objects instead of lists? Besides, data frames are the natural way to
> manipulate information in R. We'll see which is better.

Data frames are better in terms of performance :)
...
## for now, remove them with miso.hg19[[3]] <- miso.hg19[[3]][-c(49507, 49508), ]
...
misoEvents <- lapply(1:length(miso.hg19),
                     function(x) {
                         ...
                         groups <- unlist(sapply(1:length(diff),
...
library(dplyr)
events <- list(matsEvents, suppaEvents, misoEvents, vastToolsEvents)
...
fullJoinTables <- function(tables, by = NULL, toNumeric = FALSE) {
    # If there's only one table, there's no need to full join...
...
joint <- lapply(types, function(type, events) {
    ...
    res <- switch(type,
                  "SE"   = join(c("Strand", "Chromosome", "C1.end", "A1.start",
                                  "A1.end", "C2.start"),
                                c(rep(FALSE, 2), rep(TRUE, 4))),
                  "A3SS" = join(c("Strand", "Chromosome", "C1.end", "C2.start",
                                  "A1.start"), c(rep(FALSE, 2), rep(TRUE, 3))),
                  "A5SS" = join(c("Strand", "Chromosome", "C1.end",
                                  "C2.start", "A1.end"),
                                c(rep(FALSE, 2), rep(TRUE, 3))),
                  "AFE"  = join(c("Strand", "Chromosome", "C1.start",
                                  "C1.end", "A1.start", "A1.end",
                                  "C2.start"), c(rep(FALSE, 2), rep(TRUE, 5))),
                  "ALE"  = join(c("Strand", "Chromosome", "C1.end",
                                  "A1.start", "A1.end", "C2.start",
                                  "C2.end"), c(rep(FALSE, 2), rep(TRUE, 5))),
                  "MXE"  = join(c("Strand", "Chromosome", "C1.end",
                                  "A1.start", "A1.end",
                                  "A2.start", "A2.end", "C2.start"), 
                                c(rep(FALSE, 2), rep(TRUE, 6))),
                  "TandemUTR" = join(c("Strand", "Chromosome", "C2.start",
                                       "C2.end", "A1.end"),
                                     c(rep(FALSE, 2), rep(TRUE, 3))),
                  "RI"   = join(c("Strand", "Chromosome", "C1.start",
                                  "C1.end", "C2.start", "C2.end"), 
                                c(rep(FALSE, 2), rep(TRUE, 4))))
    return(res)
}, events)
names(joint) <- types
...
for (e1 in 1:length(es)) {
    for (e2 in 1:length(es)) {
...
A note of interest... `````` should be added
or else MATS events aren't matched.
...
all(junctions.files[[1]]$junction == junctions.files[[2]]$junction)
all(junctions.files[[1]]$junction == junctions.files[[3]]$junction)

# Let's parse the junction identifiers
# parseTCGAJunctions <- function(junctions) {
#     parsed <- stringr::str_split_fixed(junctions, ":|,", 6)
#     parsed <- data.frame(parsed, stringsAsFactors = FALSE)
#     parsed[[2]] <- as.numeric(parsed[[2]])
#     parsed[[5]] <- as.numeric(parsed[[5]])
#     return(parsed[ , c(1, 3, 2, 5)])
# }

junctionsID <- junctions.files[[1]]$junction
#parsed <- parseTCGAJunctions(junctionsID)

junctions <- cbind(junctions.files[[1]]$raw_counts,
                   junctions.files[[2]]$raw_counts,
                   junctions.files[[2]]$raw_counts)
names(junctions) <- junctionsID
junctions <- data.frame(junctions, stringsAsFactors = FALSE)
```

All right! Now let's search for pairs!
...
id_incA <- paste0(chr, ":", ifelse(c1<a1, c1, a1), ":", strand, ",", chr, ":", ifelse(c1>a1, c1, a1), ":", strand)
id_incB <- paste0(chr, ":", ifelse(a2<c2, a2, c2), ":", strand, ",", chr, ":", ifelse(c2>a2, c2, a2), ":", strand)
id_excl <- paste0(chr, ":", ifelse(c1<c2, c1, c2), ":", strand, ",", chr, ":", ifelse(c2>c1, c2, c1), ":", strand)

index_incA <- fastmatch::fmatch(id_incA, names(junctions))
index_incB <- fastmatch::fmatch("chr1:12697:+,chr1:13221:+", names(junctions))
index_excl <- fastmatch::fmatch("chr1:12697:+,chr1:13221:+", names(junctions))
...
> TODO: match junctions from junction files with events from all those programs