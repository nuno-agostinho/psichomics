...
| [Whippet][whippet-tutorial]            |     |     |     |     | Limited* |
...
## Prepare [Whippet][] data

psichomics supports loading inclusion levels from [Whippet][]. As Whippet
creates a file per sample, psichomics will join the information from multiple
files into a single dataset.

Move Whippet output files to a folder and then load them by following the
instructions in [Load user-provided data into psichomics][loading].
...
> Sample metadata from **SRA** can now be
[automatically loaded][sra-run-tutorial].
...
[whippet-tutorial]: #prepare-whippet-data
