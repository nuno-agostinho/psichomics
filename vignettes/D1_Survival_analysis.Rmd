...
data <- list.parse3(data.frame(x, y, mark))
data <- lapply(data, function(i)
    if(i$mark) c(i, marker = marker)
    else c(i, marker = dont))

library(highcharter)
highchart() %>%
    hc_chart(zoomType="xy") %>%
    hc_yAxis(min=0, max=1) %>%
    hc_add_series(data = c(first, data[1:10]), step="left", name="Main") %>%
    hc_add_series(data = c(first, data[11:20]), step="left", name="Non")
```
