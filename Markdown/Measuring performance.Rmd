...
evaluated and the `unit` argument to decide on the time unit: ns, us, ms, s, t, 
hz, khz, mhz, eps or f.
...
## rbenchmark
Although it also allows to compare times between chunks of code, it can be more
useful than `microbenchmark` to check the elapsed time of a program over
multiple iterations since it shows the real elapsed time. Note however that this
method is less precise than `microbenchmark`. You can modify the `replications`
argument to alter the number of times an expression is evaluated.
