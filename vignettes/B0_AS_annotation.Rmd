...
> TODO: Talk about the considerations of which alternative isoform is considered
for the exon/intron inclusion levels. Also, talk about how this is standarised
in the end (using 1-PSI to get the complementing ratio).
...
Before, I thought something along the lines of:
...
** See section "Event representation" for more information. **
...
example, the result is different based on the strand). The regular ```if``` is
not vectorised, but ```ifelse``` is. The problem with this command is that it
MUST return a value with the same length as the arguments... This is bad for
...
# maybe it's faster to fill everything and edit what's different? it's not much
# faster... actually, it takes about the same time and it's less clear
...
There's a clearer improvement! Still, I can't fathom if there's an even better,
faster way of doing this. We are separating the plus and minus operations but
maybe there's a way to do it in a single operation. Of course, we only care
about using a single operation if it's faster else there's no need to use it.

> TODO: maybe we can use `ifelse` after all! We could return a list with all the
elements instead of just the elements themselves. Would this be faster?
...
> TODO: This didn't turn out as fast as expected. Why's that? Don't forget that
VAST-TOOLS handles a greater quantity of events relative to others.
...
parseMisoSE <- function(event) {
    ...
    if (!is.null(event)) {
        ...
        if (nrow(event[plus, ]) > 0) {
            iplus <- index[plus]
            parsed[plus, ][c("C1.start", "C1.end")] <- event[iplus + 2, 4:5]
            parsed[plus, ][c("A1.start", "A1.end")] <- event[iplus + 3, 4:5]
            parsed[plus, ][c("C2.start", "C2.end")] <- event[iplus + 4, 4:5]
        }
        # Minus strand
        minus <- !plus
        if (nrow(event[minus, ]) > 0) {
            iminus <- index[minus]
            print(event[iminus + 2, 4:5])
            parsed[minus, ][c("C1.start", "C1.end")] <- event[iminus + 4, 5:4]
            parsed[minus, ][c("A1.start", "A1.end")] <- event[iminus + 3, 5:4]
            parsed[minus, ][c("C2.start", "C2.end")] <- event[iminus + 2, 5:4]
        }
        return(parsed)
    }
}
...
Awesome! We can even get better times by defining the columns in only one step. 
This also allows us to generalise the parsing of different event types into one
function which wasn't possible before given the different number of assignments
needed for each event type in the previous method.

```{r, eval=FALSE}
parseMisoSE2 <- function(event) {
    # Filter out events that aren't valid
    validator <- c("gene", "mRNA", rep("exon", 3), "mRNA", rep("exon", 2))
    event <- getValidEvents(event, validator)
    eventType <- "SE"
    ...
    if (!is.null(event)) {
        ...
        if (nrow(event[plus, ]) > 0) {
            iplus <- index[plus]
            parsed[plus, ][c("C1.start", "A1.start", "C2.start",
                             "C1.end", "A1.end", "C2.end")] <- 
                unlist(event[c(iplus + 2, iplus + 3, iplus + 4), 4:5])
        }
        # Minus strand
        minus <- !plus
        if (nrow(event[minus, ]) > 0) {
            iminus <- index[minus]
            parsed[minus, ][c("C1.start", "A1.start", "C2.start",
                              "C1.end",  "A1.end", "C2.end")] <- 
                unlist(event[c(iminus + 4, iminus + 3, iminus + 2), 4:5])
        }
        return(parsed)
    }
}
...
# MATS' exon skipping events don't match if we don't add one
...
# All right... let's separate all events based on event type and then program
library(dplyr)
library(plyr)
...
fullJoinTables <- function(tables, by = NULL, toNumeric = FALSE) {
    tables <- lapply(tables, getNumerics, by, toNumeric)
    joined <- Reduce(function(x, y) full_join(x, y, by), tables)
    return(joined)
}
...
types <- names(events)
join <- lapply(types, function(type, events) {
    print(type)
...
# for exon skippings events (just change)
j <- join$SE
programs <- j[grep("Program", names(j))]
nas <- !is.na(programs)
...
Another thing we have to remember is that different programs may consider 
different alternative exons to calculate the exon inclusion levels. So as to 
standarise and mitigate any differences between the same event across different
representations, we'll be sorting the first alternative isoform as the one
considered and used to calculate inclusion levels. For example, to compare a MXE
event, we'll say the first alternative exon is to be considered. If any program
considers the second one, we'll change the values to consider the first one
instead (simply change the inclusion level to the complement, i.e. 1-PSI).

# Differences in annotation
Different programs can have the same event annotated differently. For example,
a program can consider the first exon of a mutually exclusive exons event as the
alternative exon while the same event can be considered constitutive by another
program. This makes it hard to match different events which are indeed the same.
To overcome this, a standard for comparison is needed. A simple way to do this
is to sort the positions. In the case mentioned earlier, we could consider the
alternative exon the one with lower coordinates (note: yes, we could use the
upstream exons instead, but it's computationally faster to use an approach
independent of strands). It's simply a matter of using the complement of a given
value (1 - value) when changing the exon of reference.

There are 5 types of events where this may happen: AFE/ALE, A3/5SS, MXE and
Tandem UTR. The inclusion element in SE and RI is universal for all.

In MXE, AFE/ALE and Tandem UTR, we may consider the second exon instead of the
first as the alternative. In A3/5SS, we may consider either the longest or
shortest isoform as the alternative. Let's abstract from all that. Let's just
sort ALL the coordinates and compare if they are the same given the strand and
chromosome (we can't ignore these two attributes, of course).
...
toNumbers <- function(type, annotation) {
...
annotation <- psichomics:::getParsedAnnotation()
annot2 <- NULL
types <- names(annotation)
for (type in types) {
    coord <- sortingCoordinates(type)
    annot2[[type]] <- toNumbers(type, annotation)
    if (!is.null(coord)) {
        for (program in names(annotation[[type]])) {
            print(paste(type, program))
            table <- annot2[[type]][[program]]
            plus <- table[["Strand"]] == "+"
            plusOrd <- apply(table[plus, coord], 1, sort)
            minusOrd <- apply(table[!plus, coord], 1, sort, decreasing = TRUE)
            annot2[[type]][[program]][plus, coord] <- t(plusOrd)
            annot2[[type]][[program]][!plus, coord] <- t(minusOrd)
        }
    }
}

# Let's compare!
join <- psichomics:::joinAnnotation(annotation)
join2 <- psichomics:::joinAnnotation(annot2)

# Add C2 start from MATS and SUPPA to AFE events
suppaAFE <- join2$AFE$SUPPA.C2.start
matsAFE  <- join2$AFE$MATS.C2.start
join2$AFE[["C2.start"]] <- as.numeric(ifelse(sapply(suppaAFE, is.null),
                                             ifelse(sapply(matsAFE, is.null), 
                                                    NA, unlist(matsAFE)),
                                             unlist(suppaAFE)))

# Add C1 end from MATS and SUPPA to ALE events
suppaALE <- join2$ALE$SUPPA.C1.end
matsALE  <- join2$ALE$MATS.C1.end
join2$ALE[["C1.end"]] <- as.numeric(ifelse(sapply(suppaALE, is.null),
                                           ifelse(sapply(matsALE, is.null), 
                                                  NA, unlist(matsALE)),
                                           unlist(suppaALE)))

annot <- lapply(names(join2), 
                function(i) { 
                    join2[[i]][, c(grep("Event.ID", names(join[[i]]), 
                                        value = TRUE), 
                                   "Chromosome", "Strand",
                                   getCoordinates(i))]
                })
names(annot) <- names(join2)

annot$AFE[["C2.start"]] <- join2$AFE$C2.start
annot$ALE[["C1.end"]]   <- join2$ALE$C1.end

for (type in names(join)) {
    print(type)
    psichomics:::vennEvents(join, type)
    title(paste(type))
    psichomics:::vennEvents(join2, type)
    title(paste(type, "unique interest sorted"))
    #write.table(annot[[type]], file = paste0("data/annotation_", type, ".txt"),
...
All right! Now let's search for pairs (only in exon skipping events, for now)!
```{r TCGA/events match, eval=FALSE}
chr <- joint$SE$Chromosome
strand <- joint$SE$Strand
c1e <- joint$SE$C1.end
a1s <- joint$SE$A1.start
a1e <- joint$SE$A1.end
c2s <- joint$SE$C2.start

id_incA <- sprintf("%s:%s:%s,%s:%s:%s", chr, ifelse(c1e<a1s, c1e, a1s), strand,
                   chr, ifelse(c1e<a1s, a1s, c1e), strand)
id_incB <- sprintf("%s:%s:%s,%s:%s:%s", chr, ifelse(a1e<c2s, a1e, c2s), strand,
                   chr, ifelse(a1e<c2s, c2s, a1e), strand)
id_excl <- sprintf("%s:%s:%s,%s:%s:%s", chr, ifelse(c1e<c2s, c1e, c2s), strand, 
                   chr, ifelse(c1e<c2s, c2s, c1e), strand)
...
exon skipping events. My PI says that is quite all right but I was expecting for
...
cross-linking clinical data with sequencing files.