...
annotation files with the splicing event ID's to parse the splicing events.
...
## Skipping exon
Below, there is a tabular representation of a Skipping Exon events. The first
...
validate_event <- function(i, annotation, genes_index, validator, progress) {
  ...
  if (length(event[ , 3]) != length(validator) || !all(event[ , 3] == validator))
    return(event)
}
