...
eventGE <- assignValuePerPatient(geneExprNorm[gene, ], match, clinical,
                                  samples=unlist(tumour))
...
daysToDeath <- "days_to_death"
survTime <- getAttributesTime(clinical, event=daysToDeath, 
                              timeStart=daysToDeath)
...
for (row in min:max) {
    # Assign alternative splicing quantification to patients based on their samples
    singlePSI <- assignValuePerPatient(psi[row, ], match, clinical,
                                  samples=unlist(tumour))
