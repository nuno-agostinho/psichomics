...
sampleAttributes <- "~/Downloads/GTEx_Data_V6_Annotations_SampleAttributesDS.txt"
junctionQuant <- "~/Downloads/GTEx_junction_reads.txt"

# Check GTEx tissues available based on the sample attributes
getGtexTissues(sampleAttributes)

tissues <- c("blood", "brain")
gtex <- loadGtexData(subjects, sampleAttributes, junctionQuant, tissues)
```
...
tissues <- NULL
gtex <- loadGtexData(subjectPhenotype, sampleAttributes, junctionQuant, tissues)
```
...
# Percentage of missing values tolerated by gene: tolerated missing values 
# are replaced with the median expression of that gene
naTolerance <- 0

# Center and scale normalised gene expression values
# Perform principal component analysis
pca <- performPCA(pcaData, center=TRUE, scale.=FALSE, naTolerance=naTolerance)
