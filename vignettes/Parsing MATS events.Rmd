...
MATS outputs inclusion levels to a folder (`MATS_output`) which contain two
files: one of them only takes into account junction counts to calculate the 
inclusion levels while the other uses junction counts and reads on target
(read more about it at <http://rnaseq-mats.sourceforge.net>). Both these files
present the same columns with the exception of four that only differ in the
...
counts for a skipping exon event.
  ...
  switch(event_type,
         ...
         "A3SS" = {
           ...
           if (strand == "+") {
             ...
             parsed[["C2 end"]]  <- as.numeric(event[8])
           }
         }
  ) # end of switch
  return(parsed)
