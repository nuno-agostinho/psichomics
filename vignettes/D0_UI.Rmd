...
It seems that renderUI has a problem that conditionalPanel does not: when trying
...
Delegating the browser to download the files requires to know when the files
have finished downloading since we can't tell the browser to save in a specific
location. Also, since the user can change the default location to where the 
files are downloaded, we'll need to ask the user the location of the files.

Also, as users may opt to have their files automatically decompress after a
download, the MD5 check may fail so this step will simply present a warning in
those cases.
...
lot of glyphs! Besides, it has a few extra options as well. See the rotating
gear in the top-right corner when something is loading? It's a FontAwesome
glyph with the supported class `fa-spin` which animates the icon using CSS3.
Note however that CSS3 animations aren't supported in IE 9 or older.
...
# Groups
Should groups be right next to data input? The problem is that the user will not
understand the motivation for groups there without explaining why. The most
intuitive way would be to open a window to create/edit groups every time the 
user needs/wants to add them. For example, simply add a button to "Edit groups"
near fields that ask for groups.

Another nice thing to add: we could reflect the groups being selected by showing
a filtered data table of the current selection like this: 
...
# Auto-completing suggestions as the user types
One thing we allow users to do is to subset data directly using the `subset` 
function from R. That's a very flexible method but the user needs to have access
to the column names to properly use this feature. Of course, the user can read
all the columns by viewing the data at the right of the grouping features but 
really, what if the user wants to find a column in dozens? This could also be
used to help the user navigate the file system, listing the subdirectories of
the current directory (e.g. when asking for a download folder).

A way we could handle this is by giving suggestions to the user while typing in 
the text field along the lines of http://yuku-t.com/jquery-textcomplete/. This
method even supports partial text completion, we just need to change the 
callback function from the examples; see the following fiddle for a working demo
of this: https://jsfiddle.net/danielagos/jk74yew0/

Now, we'll have to consider how to implement this in our application in a way
that the suggestions are updated according to the group names available.

# Debug
To debug a Shiny app, you simply need to enter the command `browser()` and when
running the Shiny app, the debug mode will start when hitting that command.
...
cluster <- 3
k <- kmeans(data, cluster)

x <- split(data[, 1], k$cluster)
y <- split(data[, 2], k$cluster)

plot(data)
for (i in seq_along(x)) {
    pol <- chull(x[[i]], y[[i]])
    polygon(x[[i]][pol], y[[i]][pol], col=i)
}
points(data, col="orange")
```
            ...
            if(input$decimal < 0.5) {
                replaceData(proxy, data.frame(
                    ...
                    Value = as.character(c(input$integer, 
                                           ...
                                           input$format,
                                           input$animation)), 
                    stringsAsFactors=FALSE), rownames = FALSE)
            } else {
