...
```{r benchmark-miso-grep, eval=F}
library(microbenchmark)
microbenchmark(times = 1,
               no_lapply=matchMisoEvent1(eventID, annotation, IDcolumn),
               lapply=lapply(rep(eventID,3),
                             matchMisoEvent1, annotation, IDcolumn))
```
...
matchMisoEvent2D <- function(eventID, annotation, IDcolumn) {
  ...
  # Get index of the next gene
  next_index <- genes_index[match(index, genes_index) + 1]
  lapply(seq_along(index), getMisoEventD, annotation, index, next_index)
}
...
All right, there are six outliers from the 14957 A3SS events. Since the ID shows
the longer isoform first (chr1:6470:6628:-\@chr1:5805&#124;5810:5659), it means
the longer isoform is the one being considered as the alternative
([MISO FAQ](https://miso.readthedocs.org/en/fastmiso/#answer12). This is
important because we need to know which isoform is the inclusion level referring
to.
...
There are 2 outliers out of 12807 A5SS events. Since the ID shows
the longer isoform first (chr1:7605:7469&#124;7389:-\@chr1:7096:7231:-), it means
the longer isoform is the one being considered as the alternative
([MISO FAQ](https://miso.readthedocs.org/en/fastmiso/#answer12). This is
important because we need to know which isoform is being quantified.
...
We also need to identify which alternative exon is being considered to calculate 
...
list_mRNA2 <- function(event) {
  ...
  group <- rep(seq_along(mRNA_len), mRNA_len)
...
elem <- lapply(seq_along(gene), function(i)
...
```profvis``` while running parseMisoSE on all exon skipping events from the 
...
microbenchmark(times=5,
               lapply=elem <- lapply(seq_along(gene), function(i)
                   as.character(events[gene[i]:(next_gene[i]-1), 3])),
