...
output: 
    rmarkdown::html_vignette:
        toc: true
vignette: >
  %\VignetteIndexEntry{Loading SRA and user-provided RNA-seq data}
...
# Loading data based on RNA-seq data

The following tutorial goes through the steps required to load data based on
RNA-seq data:
...
Ensure that the SRA metadata is also available in the same folder where the
gene expression and junction quantification tables prepared for psichomics are
located.

### Load data in the visual interface
...
**Load files** to automatically scan and load supported files from that folder.

### Load data in command-line interface (CLI)

```{r, eval=FALSE}
library(psichomics)
data          <- loadLocalFiles("/path/to/psichomics/prepared/output/")
geneExpr      <- data[[1]]$`Gene expression`
junctionQuant <- data[[1]]$`Junction quantification`
sampleInfo    <- data[[1]]$`Sample metadata`
```

# Loading alternative splicing quantification data from VAST-TOOLS

After quantifying alternative splicing using VAST-TOOLS, put the inclusion
levels table (containing alternative splicing quantification values) in a new
folder.
...
```{r, eval=FALSE}
library(psichomics)
data <- loadLocalFiles("/path/to/vast-tools/folder")
junctionQuant <- data[[1]]$`Junction quantification
```
