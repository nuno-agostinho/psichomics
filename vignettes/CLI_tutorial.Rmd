...
* `parseMatsAnnotation`: Parse splicing annotation from [rMATS][2]
* `parseMisoAnnotation`: Parse splicing annotation from [MISO][3]
* `parseSuppaAnnotation`: Parse splicing annotation from [SUPPA][1]
* `parseVastToolsAnnotation`: Parse splicing annotation from [VAST-TOOLS][4]
...
from [TCGA][1], [GTEx][2] or user-owned files.

Data is downloaded from [Firebrowse][11], a service that hosts proccessed data
from [TCGA][1], as required to run the downstream analyses. Before downloading 
data, check the following options:
...
# Loading data from GTEx or user-owned files
...
[11]: http://firebrowse.org
[12]: http://www.ensembl.org
