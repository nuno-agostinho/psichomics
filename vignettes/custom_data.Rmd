...
expression output directly from VAST-TOOLS ([read here how][vast]). psichomics
also supports importing generic data from any source as long as the tables are
...
> If you have sample information from the SRA Run Selector, psichomics can
automatically load it using the visual interface or the function 
[`loadLocalFiles()`][loadLocalFiles]. Otherwise, you need to prepare the table
in a specific way for it to be readable by psichomics.
...
identify the cognate gene and coordinates) from user-provided tables, unless 
[these tables come from VAST-TOOLS][vast].
...
# Preparing tables based on RNA-seq data
...
of your interest is available in recount2, thus making it quicker to analyse
gene expression and alternative splicing for your samples of interest.
...
To load the data, move all files to a new folder (VAST-TOOLS alternative 
splicing quantification and gene expression tables and sample/subject
information) and follow the instructions in
[Load user-provided data into psichomics][loading].
...
[vast]: #preparing-vast-tools-data
