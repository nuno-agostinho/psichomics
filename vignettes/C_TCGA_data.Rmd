...
work more or less like the built-in `read` functions with minor changes (there
is a problem when trying to not assign a header if the header is not of the same
type as the content of the text... weird bug).
...
All right then, what if we separate the files by their beginning? Seems simple
...
format of interest. That script will contain an expression representing the
header of the file (maybe a regular expression or the content of the header), 
...
The format of these files could be in R. But there's advantages to using other 
language. We could then use subfolders in other R folders... So what other 
language could we use? XML? JSON? Or we could create a specific domain-specific
language like this:

```
tablename: Clinical
filename: clin.merged.txt
check: admin.bcr, admin.file_uuid
```

Let's do it in R first of all and we'll see later on.