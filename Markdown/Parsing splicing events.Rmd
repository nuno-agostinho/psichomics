...
as **parseSuppaEventC1**).

```{r parse-all}
parseSuppaEventC1 <- function(event) {
  # Split event ID by semicolon and colon symbols
  tmp <- strsplit(event, ";|:")
...
How much slower is our function with this addition? Let's find out.

```{r benchmark-parse-all}
benchmark(replications = 1,
  parseSuppaEventC(events),
  parseSuppaEventC1(events)
)
```

Well, it takes about four times more time to complete. Maybe we should try to
optimise the code we just add. What if we used the classic if/else instead of
the switch?
...
[UCSC's liftOver](https://genome.ucsc.edu/cgi-bin/hgLiftOver) to change 
the positions (but the ID's stayed the same). Therefore, we need to cross-ref
the annotation files with the splicing event ID's to parse the splicing events
and to get the expression levels.
...
match <- grep(event, annotationFile[[9]])
annotationFile[match,]
```